...
public struct QueryRelations {
    ...
    /**
     ...
     - parameter sort: The sorting method for the relationships, can be `score` or `frequency`. `frequency` is the number of unique times each entity is identified. The default is `score`.
    ...
    */
    public init(entities: [QueryRelationsEntity]? = nil, context: QueryEntitiesContext? = nil, sort: String? = nil, filter: QueryRelationsFilter? = nil, count: Int? = nil) {
...
extension QueryRelations: Codable {
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case filter = "filter"
        case count = "count"
        static let allValues = [entities, context, sort, filter, count]
    }

    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        entities = try container.decodeIfPresent([QueryRelationsEntity].self, forKey: .entities)
        ...
        filter = try container.decodeIfPresent(QueryRelationsFilter.self, forKey: .filter)
