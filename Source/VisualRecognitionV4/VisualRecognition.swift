...
public class VisualRecognition {
        ...
        completionHandler: @escaping (WatsonResponse<AnalyzeResponse>?, WatsonError?) -> Void)
    {
        ...
        if let collectionIDsData = collectionIDs.data(using: .utf8) {
            multipartFormData.append(collectionIDsData, withName: "collection_ids")
        }
        if let featuresData = features.data(using: .utf8) {
            multipartFormData.append(featuresData, withName: "features")
        }
    ...
    public func createCollection(
        ...
        trainingStatus: BaseCollectionTrainingStatus? = nil,
        headers: [String: String]? = nil,
